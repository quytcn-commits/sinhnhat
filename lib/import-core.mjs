...
export function parseEmployees(XLSX, wb, { sheet = DEFAULT_SHEET, onlyActive = true } = {}) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    // KHÔNG nạp "số ngày/giờ làm việc" từ Excel: số đó cũ dần theo thời gian.
    // Để trống → app tự tính từ joinDate (xem daysWith/hoursWith). Admin vẫn có
    // thể ghi đè thủ công qua trang quản trị khi cần.
    // Khối nhân viên → chọn nền poster: "bld" (ban lãnh đạo) / "bo" (back office)
    // / còn lại (kd / trống) = poster KD mặc định.
    const kRaw = idx.khoi >= 0 ? strip(row[idx.khoi]) : "";
    /** @type {"bld" | "bo" | "kd"} */
    const khoi = kRaw === "bld" ? "bld" : kRaw === "bo" ? "bo" : "kd";
    const emp = {
      cccd,
      fullName: up(name),
